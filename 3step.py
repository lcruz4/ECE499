...
for x in range(1,200):
  ...
  ref.ref[ha.RHP] = -.38
  ref.ref[ha.RAP] = -.19
  ref.ref[ha.LHP] = -.38+.19-.19*math.cos((state.time-t)*0.1*math.pi)
  ref.ref[ha.LAP] = 0.19-.095+.095*math.cos((state.time-t)*0.1*math.pi)
...
for x in range(1,100):
  ...
  ref.ref[ha.RAP] = -.19+.19-.19*math.cos((state.time-t)*0.2*math.pi)
  r.put(ref)
  [statuss, framesizes] = s.get(state, wait=False, last=False)
  while((state.time-t)<0.05*x):
    [statuss, framesizes] = s.get(state, wait=False, last=False)
